...
Sales Metrics:
- **Sales Growth Rate (%)**: Compare current and prior periods to identify demand trends. Calculated as (Sales in the current period in the fiscal year - Sales in the prior period in the fiscal year) Ã· Sales in the prior period in the fiscal year. A positive value indicates growth, while a negative value indicates a decline in sales.
...
- **Gross Margin (%)**: Assess profitability by comparing revenue to cost of goods sold. Calculated as((Net Sales - COGS) Ã· Net Sales. A higher percentage reflects better profitability.
- **Return Rate (%)**: Monitor product quality and customer satisfaction through return ratios.  Calculated as Returned Quantity Ã· Total Sold Quantity. A lower percentage indicates fewer returns and higher product acceptance.
...
The task pane appears as an additional panel in the user interface and can be opened from anywhere in Business Central. Tasks include suggestions, validations, followâ€‘ups access to draft documents, and other agentâ€‘initiated actions. Users can review task details, navigate directly to the affected record, complete tasks, or dismiss them.

This improvement primarily benefits roles that rely heavily on AIâ€‘driven workflows â€” such as accountants, sales people, and managers â€” who often engage with multiple agents throughout the day. The task pane reduces friction by collecting all insights in a single location, minimizing missed tasks and improving throughput.
...
Business Central introduces a new version of the OIOUBL eâ€‘invoicing format designed for a Danish pilot program that supports carbonâ€‘footprint data exchange. The format automatically includes emissions values sourced from the **Sustainability** module for both sales and purchase processes. When sending sales eâ€‘invoices, Business Central embeds the calculated emission data from the **Item Card** or **Resource Card** pages, or the  updated **Sales Lines**. When receiving purchase eâ€‘invoices, the footprint values are captured and added to purchase invoices or purchase orders and posted to sustainability ledger entries.

You just need to select the new format on the **Eâ€‘Documents Service** page. No additional configuration is required. This builds the foundation for future sustainability data exchange across the full transaction lifecycle.

### Align VAT periods with Microsoft standards for Czechia

The feature consolidates the Czech localization functionality **VAT Period** into the standard **VAT Return Period** table. All processes previously based on VAT Periodâ€”such as period generation, closing, and posting checksâ€”will now operate on VAT Return Period. 
...
BCâ€‘Bench is a benchmarking framework for evaluating agent performance on realâ€‘world Dynamics 365 Business Central AL coding tasks. Inspired by SWEâ€‘Bench, it provides measurable, repeatable results instead of subjective impressions, helping developers understand what improvements actually work.
...
By producing transparent metrics, BCâ€‘Bench increases trust in Copilot capabilities, supports dataâ€‘driven investment decisions, and helps partners and the community clearly understand an agent's strengths and limitations for Business Central development.
...
*   Business Central sends updated onâ€‘hand quantities using the new *inventorySetQuantities* mutation.
*   The connector avoids concurrency conflicts by opting out of Shopifyâ€™s compareâ€‘andâ€‘swap mechanism, because Business Central acts as the source of truth.
*   The connector automatically retries operations when Shopify responds with concurrencyâ€‘related errors, helping you reduce missed updates. When retries still fail, the system logs the skipped records so you can review and take corrective action.
...
You can now work with Shopifyâ€™s new **article\_reference** metafield type. This helps you maintain richer product information when you synchronize item data.
...
When you create new items by syncing products from Shopify, Business Central now imports the productâ€™s HTML description into the **Marketing Text** field, provided that you enable **Sync Item Marketing Text**. This extra synchronization gives you richer product information on new items without manually copying descriptions.

### Set the unit of measure when you select an item on order lines

When you map Shopify order lines to items manually, Business Central now automatically fills in the **Unit of Measure Code** based on the value from the itemâ€™s setup.

*   If the item has a **Sales Unit of Measure**, that value is used.
*   Otherwise, the **Base Unit of Measure** is applied.

The auto-fill reduces manual entry and helps you avoid unitâ€‘ofâ€‘measure errors when you process Shopify orders.
...
You can control whether Business Central creates sales documents from Shopify orders in the shop currency or in the customerâ€™s checkout (presentment) currency. This gives you full flexibility when working with Shopify Markets or any store that sells in multiple currencies.

A new **Currency Handling** setting on the **Shopify Shop Card** lets you choose:

- **Shop currency** (default)Â 
...
Business Central extends vendor templates to include the **Eâ€‘Document Type** field that's used in the **Eâ€‘Documents** module. Instead of always defaulting new vendors to **Purchase Order**, administrators can now specify whether a vendor should default to **Purchase Order** or **Purchase Invoice** based on their processes. When you create a vendor from a template, Business Central automatically applies the selected type, ensuring consistent setup and reducing errors in downstream eâ€‘document handling. This enhancement improves efficiency, supports scalable vendor onboarding, and aligns vendor configuration with realâ€‘world document workflows. To start using this feature, on the **Vendor Template** page, choose the **Receive E-Document To** option on the **Receiving** FastTab.
...
This feature extends Company Information with the same Service Participant model already used for customers and vendors. By aligning these entities, Business Central gains a more consistent and futureâ€‘proof approach to identifying companies in electronic document exchange scenarios.

**Objectives**

* Provide a reliable and localizationâ€‘agnostic way to identify the company when sending or receiving Eâ€‘Documents.
* Align Company Information with the established Service Participant framework used for customers and vendors.
* Improve interoperability with multiple standards, including Peppol and other countryâ€‘specific schemes.
* Simplify setup and reduce errors caused by inconsistent identification methods.

To enable it, administrators need to navigate to **Company Information** and specify the appropriate **Service Participant Identifier** and **Scheme**. When Business Central sends or receives Eâ€‘Documents, it uses the new **Service Participant** fields for company identification. The system behavior is now aligned with how customer and vendor participants are handled, increasing consistency across the application.

### Link inbound e-documents to purchase invoices

This feature helps when inbound eâ€‘document arrives from the connector after the related purchase invoice is already in Business Central, specifically in intercompany processes.

To link an eâ€‘document, you open the **Eâ€‘Document Purchase Draft** page, ensure a vendor is assigned, and then use the **Link to Existing Document** action. Business Central displays a filtered list of purchase invoices or credit memos based on vendor and total amount. After you select a document, Business Central updates it with the eâ€‘document totals and sets the eâ€‘document status to **Processed**.

You can also relink an already processed eâ€‘document to a different document. Depending on how you created the previous document, Business Central either unlinks it or deletes it. The feature requires that the vendor has an **IC Partner Code**, because it currently supports intercompany scenarios only.

### Use self-billed invoices

Business Central offers full support for selfâ€‘billed invoices in the purchase process. Buyers can now generate invoices on behalf of a vendor when a selfâ€‘billing agreement is in place. A new Boolean field on the **Vendor Card** page enables this agreement, and all **Purchase Invoices** created for that vendor, whether from **Purchase Orders** or directly, are automatically marked as selfâ€‘billed. Vendors can also be assigned a dedicated **No. Series** for selfâ€‘billed invoices, ensuring correct document sequencing.

A new report layout, **Posted Purchase Invoice â€“ Selfâ€‘Billing Invoice**, formats the invoice to align with the sales invoice style, showing buyer company information, vendor details, VAT information, and all essential invoice data.

For Danish localizations, the posting process can automatically create **Digital Vouchers** when you enable selfâ€‘billing, ensuring compliance with local audit and documentation rules. To prevent conflicts between buyerâ€‘issued and supplierâ€‘issued documents, vendors with a selfâ€‘billing agreement can't receive inbound eâ€‘documents for accounts payable. This guarantees data integrity and avoids duplicate or contradictory documents in the purchase flow.
...
Business Central cloud migration tooling is extended to let developers build fast, repeatable, and reliable data migration engines for SQLâ€‘based source systems. These custom migration engines are implemented as extensions and can be packaged, installed, and reused through AppSource, enabling consistent delivery across customer projects.

The platform introduces a generic, systemâ€‘level migration module that integrates directly with the Cloud Migration wizard through a new Generic Custom migration type. Developers implement a standardized AL interface that defines how data is mapped, replicated, and transformed from external SQL sources into Business Central. The platform provides shared helper methods to simplify custom implementations, including mapping helpers, replication and transformation orchestration, and a generic UI for tracking migration progress and errors.

Custom migration engines built using this model have the same core migration capabilities as Microsoftâ€‘provided migration engines for Business Central, Dynamics GP, and Dynamics SL. Migration execution, status, and failures surface in a generic migration UI, ensuring a consistent administrative experience across builtâ€‘in and partnerâ€‘provided migrations.

This extensibility model allows partners and ISVs to create reusable, supported migration solutions for legacy and industryâ€‘specific systems, reducing project risk and eliminating the need for adâ€‘hoc scripts, external tooling, or oneâ€‘off migration code.
...
2026 release wave 1 starts the preview of the MCP (Model Context Protocol) server that exposes Business Central Admin Center APIs in a standardized, selfâ€‘describing way. MCPâ€‘compatible AI agents can discover available admin operations, understand context, and perform environment management tasks.

Using tools like Visual Studio Code, administrators can ask questions such as environment update status, available upgrades, or installed extensions, and receive structured responses sourced directly from the Admin Center. Agents can also propose followâ€‘up actionsâ€”such as copying an environment to a sandbox, scheduling an update, or investigating a failed upgradeâ€”while keeping the user in control through explicit confirmation steps.
...
For example, even if all customers are local and expect documents in the local language, employees working in a different UI language could inadvertently generate invoices in a foreign languageâ€”making them difficult for customers to read.

To address this, weâ€™ve introduced a new Default Language Code field on the Company Information page.

You can now define a company-wide default language for customer-facing documents.
Local customers will consistently receive invoices and other sales documents in the expected language, regardless of the userâ€™s UI language.
...
- It doesnâ€™t work with orders that have prepayments, or lines of the Item Charges type.
- You canâ€™t use it with orders linked to projects, subcontracting, blanket orders, or intercompany transactions.
- You canâ€™t use auto-receive with partially received lines, or locations that use directed put away and pick (in warehouse management).
...
3.  Search for **â€œWarn about non-certified production BOMs and routingsâ€**.
...
Business Central adds new ESGâ€‘focused report layouts for sales documents, enabling you to include carbonâ€‘footprint information directly on invoices and quotes. These Word-layouts extend existing invoice and quote templates by adding COâ‚‚e information per unit and a total emissions line.
...
* **Sales Invoice**
  ...
  * Standard ESG Sales Invoice â€“ Blue (Word)
