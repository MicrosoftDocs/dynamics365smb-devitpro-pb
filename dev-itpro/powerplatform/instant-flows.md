...
|It's a flow that was shared with you but with read-only permissions|Ask the owner of the flow to add as an owner of the flow. For more information, see [Share a flow in Power Automate](/power-automate/create-team-flows).|Flows shared to users as read-only won't appear in the **Automate** action group for the users.  |
