...
The SQL queries generated by the Business Central server are of OLTP type (many, small transactions). It is therefore recommended running Business Central with `MAXDOP` set to the value 1 as a starting point. Depending on the number of cores on your database server, you can increase this number. It is not recommended to set `MAXDOP` to 0 as one query might use all resources on the databases hereby providing lower availability for other queries.
