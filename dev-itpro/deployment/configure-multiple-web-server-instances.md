...
Although you can use the [!INCLUDE[prodsetup](../developer/includes/prodsetup.md)] Setup wizard to install the [!INCLUDE[nav_web_server](../developer/includes/nav_web_server_md.md)] and create a single web server instance in IIS for the [!INCLUDE[nav_web](../developer/includes/nav_web_md.md)], there may be scenarios when you want to set up multiple instances. For example, you could set up a separate [!INCLUDE[nav_web](../developer/includes/nav_web_md.md)] instance for the different companies of a business. For this scenario, you can use the [!INCLUDE[webserver](../developer/includes/webserver.md)] PowerShell cmdlets, which are outlined in the following table.
...
The [!INCLUDE[webserver](../developer/includes/webserver.md)] cmdlets are contained in the PowerShell script module **NAVWebClientManagement.psm1**, which is available on the [!INCLUDE[navnow](../developer/includes/navnow_md.md)] installation media (DVD).

The module is installed with the [!INCLUDE[nav_server_md](../developer/includes/nav_server_md.md)] or the [!INCLUDE[nav_web_server_md](../developer/includes/nav_web_server_md.md)].

There are different ways to launch this module and start using the cmdlets:

- If you are working on the computer where the [!INCLUDE[nav_server_md](../developer/includes/nav_server_md.md)] was installed, run the [!INCLUDE[nav_shell_md](../developer/includes/nav_shell_md.md)] as an administrator.
...
- If you installed the [!INCLUDE[nav_web_server_md](../developer/includes/nav_web_server_md.md)], just start Windows PowerShell as an administrator.
...
To create a new web server instance, you need access to the **WebPublish** folder that contains the content files for the [!INCLUDE[nav_web_md](../developer/includes/nav_web_md.md)].

- This folder is available on the [!INCLUDE[navnow](../developer/includes/navnow_md.md)] installation media (DVD) and has the path "DVD\WebClient\Microsoft Dynamics NAV\110\Web Client\WebPublish". 

- If you installed the [!INCLUDE[nav_web_server_md](../developer/includes/nav_web_server_md.md)], this folder has the path "[!INCLUDE[navnow_install_md](../developer/includes/navnow_install_md.md)]\Web Client\WebPublish".
...
SubSite example:
    ...
    ```  
    New-NAVWebServerInstance -WebServerInstance MyWebApp -Server NAVServer -ServerInstance NAVServerInstance -SiteDeploymentType Subsite -ContainerName MySiteContainer -WebSitePort 8081 -PublisherFolder "C:\WebClient\WebPublish"
    ```  

-  Susbtitute *MyWebClient* with the name that you want to give the web application in IIS for the web server instance. If you are creating a SubSite deployment type, this name will become part of the URL for opening the [!INCLUDE[nav_web](../developer/includes/nav_web_md.md)] application, for example, http://MyWebServer:8081/MyWebApp.  
  
- Susbtitute *NAVServer* to the name of the computer that is running the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] to which you want to connect.  
  
-   Susbtitute *NAVServerInstance* with the name of the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] instance to use.
...
-  Substitute *C:\WebClient\WebPublish* with the path to your WebPublish folder. By default, the cmdlet looks in the'[!INCLUDE[navnow_install_md](../developer/includes/navnow_install_md.md)]\Web Client' folder. So if you are working on a computer where the [!INCLUDE[nav_web_server_md](../developer/includes/nav_web_server_md.md)] are installed, you do not have to set this parameter.
