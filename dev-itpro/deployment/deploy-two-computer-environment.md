...
|Determine the TCP ports for the [!INCLUDE[nav_web](../developer/includes/nav_web_md.md)], client services, and SOAP/OData web services (optional) and allow communication on the port through Windows Firewall.|[!INCLUDE[prodsetup](../developer/includes/prodsetup.md)] creates a website on IIS. During Setup, you'll have to choose the port to use for the site. The default port is port 8080.<br /><br />The default client services port is 7085 (7046 in version 20 and earlier).<br /><br />If you'll enable SOAP and OData web services, you'll also need to specify a port for each. The default ports are 7047 and 7048.<br /><br />For SOAP/OData web services, you have to have an inbound rule in Windows Firewall that allows communication on the ports. You can choose to have [!INCLUDE[prodsetup](../developer/includes/prodsetup.md)] to automatically create the inbound rule in Windows Firewall. Otherwise, you'll have to do this manually. For the client services port, only allow communication through the Windows Firewall for scenario 2, where the web server and [!INCLUDE[server](../developer/includes/server.md)] are on different machines. You have to add the inbound rule for the client services port manually, and we recommend that you limit access to the port for security reasons. |[Identity and access management](/windows/security/identity-protection) in the Windows documentation.<br><br>[Limiting access to client services port](../security/enhancing-server-instance-security.md#limiting-port-access)|
