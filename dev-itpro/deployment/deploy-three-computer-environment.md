...
|Determine the TCP ports for the [!INCLUDE[nav_web](../developer/includes/nav_web_md.md)], client services, and SOAP/OData web services (optional) and allow communication on the port through Windows Firewall.|[!INCLUDE[prodsetup](../developer/includes/prodsetup.md)] creates a website on IIS. During Setup, you will have to choose the port to use for the site. The default port is port 8080.<br /><br />The default client services port is 7085. For the client services port, you must allow communication through the Windows Firewall because the web server and [!INCLUDE[server](../developer/includes/server.md)] are on different machines. In this case, you have to manually add the inbound rule for the client services port. We recommend that you limit access to the port for security reasons. [Learn how to limit port access](../security/enhancing-server-instance-security.md#limiting-port-access).<br /><br />If you'll enable SOAP and OData web services, you'll also need to specify a port for each. The default ports are 7047 and 7048. You have to add an inbound rule in Windows Firewall that allows communication on the ports. You can choose to have [!INCLUDE[prodsetup](../developer/includes/prodsetup.md)] to add the inbound rule in Windows Firewall or do it manually.|[Identity and access management](/windows/security/identity-protection) in the Windows documentation.| 
