---
title: "Technical Upgrade From Version 14 to Version 20"
description: Describes how to do a technical upgrade from Business Central 14 to 20
ms.custom: na
ms.date: 03/09/2022
ms.reviewer: na
ms.suite: na
...
author: jswymer
---
# Technical Upgrade From Version 14 to Version 20
...
1. Upgrade to Business Central Spring 2019 (version 14).
   ...
   If your solution is already on version 14, then you don't have to upgrade to the latest version 20 update.
...
1. Before you install version 20, it can be useful to create desktop shortcuts to the version 14.0 tools, such as the [!INCLUDE[admintool](../developer/includes/admintool.md)], [!INCLUDE[adminshell](../developer/includes/adminshell.md)], and [!INCLUDE[devshell](../developer/includes/devshell.md)] because the Start menu items for these tools will be replaced with the version 20.0 tools.

2. Install all components of version 20.

    You keep version 14.0 installed, because you'll need it to complete the C/AL to AL conversion. So when you install version 20, you must either specify different port numbers for components during installation or stop the version 14.0 [!INCLUDE[server](../developer/includes/server.md)] instance before you run the installation. Otherwise, you'll get an error that the [!INCLUDE[server](../developer/includes/server.md)] failed to install.

    For more information, see [Installing Business Central Using Setup](../deployment/install-using-setup.md).

3. Copy the version 14 **CodeViewer** add-in to the version 20.0 server installation
...
Version 20 introduces the capability to define permissions sets as AL objects, instead of as data. Permissions sets as AL objects is now the default and recommended model for permissions. For now, you can choose to use the legacy model, where permissions are defined and stored as data in the database. Whichever model you choose, there are permission set-related tasks you'll have to go through before and during upgrade.
...
In this task, you'll publish extensions to the version 20.0 server instance. Publishing adds the extension to the application database that is mounted on the server instance. The extension is then available for installing on tenants later. It updates internal tables, compiles the components of the extension behind-the-scenes, and builds the necessary metadata objects that are used at runtime.

The steps in this task continue to use the [!INCLUDE[adminshell](../developer/includes/adminshell.md)] for version 20.0 that you started in the previous task.
...
2. Install custom base application extension on the tenant.
    ...
    At this point, the base application is upgraded to the version 20 platform and is operational. You can open the application in the client.
...
Now, your application is fully upgraded to the version 20 platform.
