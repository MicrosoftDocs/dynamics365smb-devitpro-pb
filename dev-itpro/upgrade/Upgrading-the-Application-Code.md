...
**Applies to:** [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)]. [See [!INCLUDE[nav2017](../developer/includes/nav2017.md)] version](Upgrading-the-Application-Code-2017.md).

Typically, customers want all the customizations that have been implemented in their existing [!INCLUDE[navnow](../developer/includes/navnow_md.md)] databases to be migrated to their new [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] databases. Depending on the version of [!INCLUDE[navnow](../developer/includes/navnow_md.md)] that a database is being upgraded from, the amount of code changes between the two versions can vary. To upgrade the application code, you must merge code from different versions of the application. This merge process is known as a *code upgrade* or *application upgrade*. You must upgrade the application before you upgrade the data.  
...
For the application portion of the upgrade, you must analyze and process code changes by comparing and merging three separate versions of the [!INCLUDE[navnow](../developer/includes/navnow_md.md)] database:  

|Version|[!INCLUDE[bp_tabledescription](../developer/includes/bp_tabledescription_md.md)]|  
|-------------|---------------------------------------|  
|*Original version*|This is the baseline version of the solution that you want to upgrade, such as the original release of [!INCLUDE[navcorfu](../developer/includes/navcorfu_md.md)] or [!INCLUDE[nav2017](../developer/includes/nav2017.md)].|  
|*Modified version*|This is the version that you want to upgrade, such as a customer's [!INCLUDE[navcorfu](../developer/includes/navcorfu_md.md)] or [!INCLUDE[nav2017](../developer/includes/nav2017.md)] database with customizations and add-on solutions.|  
|*Target version*|This is the target of the merge process that you want to upgrade your application to, such as the standard version of the [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] database.|  

When you merge the application objects from these three versions, you can import the result into a new [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] database that then contains the upgraded application. At the end of the process, you export the merged [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] objects from this database to a .fob file that you will use during the data upgrade.  

### Different ways of upgrading application code
You can use any tool or set of tools to help you compare and merge code.  [!INCLUDE[navnow](../developer/includes/navnow_md.md)] includes [!INCLUDE[wps_2](../developer/includes/wps_2_md.md)] cmdlets and sample scripts that can help you upgrade your application. The cmdlets are available through the [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)], or by importing the Microsoft.Dynamics.NAV.Model.Tools.psd1 module into the Windows PowerShell Integrated Scripting Environment (ISE). You can find the sample scripts on the product installation media, in the *WindowsPowerShellScripts\ApplicationMergeUtilities* folder.
...
To complete the tasks in this article, you will use various tools and components of the old [!INCLUDE[navnow](../developer/includes/navnow_md.md)] version and [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)]. Ensure that you have the following installed:
...
|[!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] |<ul><li>[!INCLUDE[nav_server_md](../developer/includes/nav_server_md.md)]</li><li>[!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)]</li><li>[!INCLUDE[nav_shell_md](../developer/includes/nav_shell_md.md)]</li>[!INCLUDE[nav_dev_long](../developer/includes/nav_dev_long_md.md)]</li> </ul>|
...
-   Use the [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)] version that matches the application database version. This is the way that is described in the tasks of this article. Note that the [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)] is not available for [!INCLUDE[nav7long](../developer/includes/nav7long_md.md)] and [!INCLUDE[navsicily](../developer/includes/navsicily_md.md)]. For these versions, you must use [!INCLUDE[nav_dev_short](../developer/includes/nav_dev_short_md.md)] or finsql.exe.
...
1.  Create four folders on the computer, and name them as follows:  
 ...
 -   **ORIGINAL**  
        ...
        This folder will be used to store the application object text file(s) from the baseline version, such as the original release of [!INCLUDE[navcorfu](../developer/includes/navcorfu_md.md)] or [!INCLUDE[nav2017](../developer/includes/nav2017.md)].  
 ...
 -   **TARGET**  
        ...
        This folder will be used to store the application object text file(s) from [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)].  
...
2.  Export all application objects from the original version of the old database, such as the original [!INCLUDE[nav2017](../developer/includes/nav2017.md)] database. Do not export system tables, which have the IDs in the 2000000000 range. Name the file **OldBaseVersion.txt**, and then save the file in the **ORIGINAL** folder that you created earlier.  
...
3.  Export all application objects, except system tables, from the old modified version, such as the customer's customized [!INCLUDE[nav2017](../developer/includes/nav2017.md)] database. Name the file **OldCustomVersion.txt**, and then save the file in the **MODIFIED*** folder that you created earlier.
    ...
    > [!TIP]  
    >  In some cases, existing customizations might be irrelevant after the upgrade because they correspond to new functionality in [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)].

4.  Export all application objects, except system tables, from the new base version, such as the original [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] database. Name the file **NewBaseVersion.txt**, and then save the file in the **TARGET** folder that you created earlier.  

    For example, using the [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)] for [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)], run the following command:
    ```  
...
1. Run the [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)] for [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] as an administrator.
...
3.  Import the new merged application object text files (.TXT) from the **Result** folder into the new database.
    ...
    -   Use the [!INCLUDE[navnowlong_md](../developer/includes/navnowlong_md.md)] [!INCLUDE[nav_dev_short](../developer/includes/nav_dev_short_md.md)].
    ...
    -   Use the [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)] (or Microsoft.Dynamics.NAV.Model.Tools.psd1 module).
...
4.  Connect a [!INCLUDE[prodshort](../developer/includes/prodshort.md)] Server instance to the database.  
    ...
    You can do this with the [!INCLUDE[nav_admin](../developer/includes/nav_admin_md.md)] or the [Set-NAVServerConfiguration cmdlet](https://go.microsoft.com/fwlink/?linkid=401394) in the [!INCLUDE[nav_shell_md](../developer/includes/nav_shell_md.md)]. In addition, you must add the service account that is used by the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] instance as a member of the **db\_owner** role in the [!INCLUDE[navnow](../developer/includes/navnow_md.md)] database on SQL Server.  

    For more information about how to do this using the [!INCLUDE[nav_admin](../developer/includes/nav_admin_md.md)], see [How to: Connect a Microsoft Dynamics NAV Server Instance to a Database](How-to--Connect-a-Microsoft-Dynamics-NAV-Server-Instance-to-a-Database.md) and [Giving the account necessary database privileges in SQL Server](Provisioning-the-Microsoft-Dynamics-NAV-Server-Account.md#dbo).  

6.  Compile all the newly imported objects.

    You can use the [!INCLUDE[nav_dev_short](../developer/includes/nav_dev_short_md.md)] or finsql.exe. For more information, see [Compiling Objects](compiling-objects.md).

    If you use the [!INCLUDE[nav_dev_short](../developer/includes/nav_dev_short_md.md)], you will first have to set it to use the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] instance that connects to the database. For more information, see [How to: Change the Microsoft Dynamics NAV Server Instance](How-to--Change-the-Microsoft-Dynamics-NAV-Server-Instance.md) or [Database Information](uiref/-$-S_2349-Database-Information-$-.md).  
...
Now, you must export all objects of the new database to an **objects.fob** file so that you can import them when performing the data upgrade. The export must include customized objects, upgraded reget-helpports, and all other [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] objects.

As with exporting objects in Task 1, you can use either the [!INCLUDE[nav2018_md](../developer/includes/nav2018_md.md)] [!INCLUDE[nav_dev_short](../developer/includes/nav_dev_short_md.md)], finsql.exe, or [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)].

With the [!INCLUDE[nav_dev_shell_md](../developer/includes/nav_dev_shell_md.md)], you can run a command that is similar to the following:
