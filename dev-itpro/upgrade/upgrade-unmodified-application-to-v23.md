...
# Upgrading Microsoft System and Base Application to Version 23
...
[![Upgrade on unmodified Business Central application.](../developer/media/bc23-upgrade-unmodified-app.png)](../developer/media/bc23-upgrade-unmodified-app.png#lightbox)  
...
$NewBCVersion = "The version number for the current System, Base, and Application extensions that you'll reinstall, for example: 23.1.24582.0"
...
1. Download the latest available update for Business Central 2023 release wave 2 (version 23) that is compatible with your current version.
...
2. Before you install version 23, it can be useful to create a desktop shortcut the [!INCLUDE[admintool](../developer/includes/admintool.md)] for the current version. The reason is that the Start menu item for the current version will be replaced with item for the [!INCLUDE[admintool](../developer/includes/admintool.md)] for version 23.

3. Install Business Central version 23 components.

    You keep the current version installed to complete some steps in the upgrade process. When you install version 23, you must either specify different port numbers for components (like the [!INCLUDE[server](../developer/includes/server.md)] instance and web services) or you must stop the current version's [!INCLUDE[server](../developer/includes/server.md)] instance before you run the installation. Otherwise, you'll get an error that the [!INCLUDE[server](../developer/includes/server.md)] failed to install.
...
1. Start [!INCLUDE[adminshell](../developer/includes/adminshell.md)] for version 23 as an administrator.
...
5. Publish new versions of 3rd-party extensions. Be sure to include new extensions that contain custom permission sets as AL objects.
    ...
    If you have new versions of these extensions, built on the Business Central version 23, then publish the new versions.  

    ```powershell
    Publish-NAVApp -ServerInstance $NewBcServerInstance -Path "<path to extension>"
    ```

6. Recompile extensions not built on version 23.

    [!INCLUDE[repair_runtime_packages](../developer/includes/repair_runtime_packages.md)]

    Do this step for any published extension versions that aren't built on version 23, and you want to reinstall on tenants. These extensions must be recompiled to work with version 23. To recompile the extensions, use the [Repair-NAVApp](/powershell/module/microsoft.dynamics.nav.apps.management/repair-navapp) cmdlet:
...
4. Synchronize the tenant with the Business Central Base Application extension.
   ...
   > [!IMPORTANT]
   > If you're upgrading a Czech (CZ) language version, you must use the `-Mode ForceSync` parameter to force synchronize the base application; otherwise, synchronization errors occur. For more information, go to [Removed table fields in base application cause sync errors](known-issues.md#removed-table-fields-in-base-application-cause-sync-errors).
