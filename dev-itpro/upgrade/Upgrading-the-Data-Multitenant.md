...
2. (Dynamics NAV upgrade only) Uninstall all V1 extensions.
    ...
    2. For each extension, run the [Uninstall-NAVApp](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.apps.management/uninstall-navapp) cmdlet to uninstall it:
...
3. Dismount the tenant.
    ...
    Before you upgrade the tenant, you must dismount it from the old server instance. To dismount the tenant, run the [Dismount-NAVTenant](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.management/dismount-navtenant) cmdlet:
...
1. Mount the tenant.
    ...
    Mount the tenant on the new [!INCLUDE[server](../developer/includes/server.md)] instance that connects to the newly application database. To mount the tenant, use the [Mount-NAVTenant](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.management/mount-navtenant) cmdlet:
...
2. Synchronize the tenant.
    ...
    Synchronize the tenant database schema with validation by running the [Sync-NAVTenant](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.management/sync-navtenant) cmdlet from the [!INCLUDE[adminshell](../developer/includes/adminshell.md)]. 
...
3. If there are published extensions with newer versions than on the tenant, synchronize all published extensions with the tenant database.
    ...
    Synchronize the schema with the database by running the [Sync-NAVApp](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.apps.management/sync-navapp) cmdlet for each extension version:
...
4. Run the data upgrade.
    ...
    You can start the data upgrade by running the  run [Start-NavDataUpgrade](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.management/start-navdataupgrade) cmdlet the [!INCLUDE[adminshell](../developer/includes/adminshell.md)]:
    ...
    To view the progress of the data upgrade, you can run [Get-NavDataUpgrade](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.management/get-navdataupgrade) cmdlet with the `–Progress` switch. 
...
5. Install extensions on the tenant.
    ...
    Install the desired extensions on the tenant by running the [Install-NAVApp](https://docs.microsoft.com/powershell/module/microsoft.dynamics.nav.apps.management/install-navapp) cmdlet:
