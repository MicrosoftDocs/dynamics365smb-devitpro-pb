---
title: (v1.0) Update customer defaultDimensions
description: (v1.0) Updates the customer default dimensions in Dynamics 365 Business Central.
...
# Update customer defaultDimensions (v1.0)
...
```json
PATCH https://{businesscentralPrefix}/api/v1.0/companies({companyId})/customers({customerId})/defaultDimensions({customerId},{dimensionId})
```
