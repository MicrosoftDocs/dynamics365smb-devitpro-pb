...
Remove calculated fields from lists if they aren't needed, especially on larger tables. Setting the field's properties to Enabled or Visiable to false is not enough, the field definition needs to be removed from the page or page extension definition. Also, if indexing is inadequate, calculated fields can significantly slow down a list page.
