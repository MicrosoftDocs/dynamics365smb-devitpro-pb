...
Updates the properties of a subscriptions object for [!INCLUDE[d365fin_long_md](../../includes/d365fin_long_md.md)].
...
```
PATCH /microsoft/automation/{apiVersion}/companies({{companyId}})/subscriptions({id})
```
...
```json
PATCH https://microsoft/automation/{apiVersion}/companies({{companyId}})/subscriptions({id})
Content-type: application/json
{
    "subscriptionId" : ,
    "notificationUrl" :
