...
Telemetry from [!INCLUDE[prod_short](../developer/includes/prod_short.md)] is stored in Azure Application Insights in the  tables *traces* and *pageViews*. This article has information and links to resources to get started learning about the language _Kusto Query Language_ (KQL) that is used to query data in Azure Application Insights.
...
* PowerShell (using the REST api). See an example here: [Powershell samples](https://github.com/microsoft/BCTech/tree/master/samples/AppInsights/Powershell)
