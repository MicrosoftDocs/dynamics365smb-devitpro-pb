...
|alHttpTimeout|Specifies the duration to wait before the request times out. The value has the format hh:mm:ss. The timeout is defined either by the **NavHttpClientMaxTimeout** configuration setting on the [!INCLUDE[server](../developer/includes/server.md)] instance or a [TimeOut method](../developer/methods-auto/httpclient/httpclient-timeout-method.md) call the extension code. The Timeout method takes precedence.|
...
|endpoint|Specifies the endpoint for the request. The endpoint is truncated to include the base URI only.|
