...
The early access preview period runs simultaneous with the update period, i.e. the early access preview period for the major version due to come out in April starts when the update period for the October major version starts. During the early access preview period, only tenants that use the [Partner Sandbox](environment-types.md#partnersandbox) license can create new sandbox environments on the preview version. The only environment localizations supported for early access preview are DE, GB, and US. 
