...
1. Find the configuration settings that you want to change, and then change the values.
   ...
   Refer to the [Settings](configure-web-server.md#Settings) section for a description of each setting.
1. When you're done making changes, save the file.
1. Restart the [!INCLUDE[webserver](../developer/includes/webserver.md)] instance for the changes to take effect.
...
1. Depending on your installation, run the [!INCLUDE[devshell](../developer/includes/devshell.md)] or Windows PowerShell as an administrator.
    ...
    Learn more in [Get started with the [!INCLUDE[webserver](../developer/includes/webserver.md)] cmdlets](../deployment/configure-multiple-web-server-instances.md#GetStartedWebServerCmdlets).

1. For each setting that you want to change, at the command prompt, run the following command:
    ...
    - `[Type]` with the site deployment type either `SubSite` or `RootSite`.

       > [!NOTE]
       > Although `-SiteDeploymentType` isn't a required parameter, the PowerShell script module **NAVWebClientManagement.psm1** is written such that the default value for `SiteDeploymentType` is `SubSite`. If `SiteDeploymentType` isn't provided, the cmdlet will only operate against subsites.
...
|ClientServicesPort|Specifies the Transmission Control Protocol (TCP) port for the [!INCLUDE[server](../developer/includes/server.md)]. The port is part of the [!INCLUDE[server](../developer/includes/server.md)]'s URL.<br /><br /> Values: 1-65535<br /><br /> Default value: 7085 (7046 in version 20 and earlier)|  
|ClientServicesProtectionLevel|Specifies the security services used to protect the data stream between the [!INCLUDE[webserver](../developer/includes/webserver.md)] and [!INCLUDE[server](../developer/includes/server.md)]. This value must match the value that is specified in the [!INCLUDE[server](../developer/includes/server.md)] configuration file. Learn more in [Configuring Business Central Server](configure-server-instance.md).<br /><br /> Values: EncryptAndSign, Sign, None<br /><br /> Default value: EncryptAndSign<br /><br />[!INCLUDE[2022rw1_and-earlier_only](../developer/includes/2022rw1_and-earlier_only.md)]| 
...
|Designer|Specifies whether the in-client designer is enabled in the [!INCLUDE[webclient](../developer/includes/webclient.md)]. Set to `true` to enable designer.<br /><br />Learn more in [Use Designer](../developer/devenv-inclient-designer.md).|
|DnsIdentity|Specifies the subject name or common name of the service certificate for [!INCLUDE[server](../developer/includes/server.md)].<br /><br /> This parameter is only relevant when security certificates are used on the [!INCLUDE[webserver](../developer/includes/webserver.md)] and [!INCLUDE[server](../developer/includes/server.md)] to protect communication. **Note:**  You can find the subject name by opening the certificate in the Certificates snap-in for Microsoft Management Console \(MMC\). The MMC is available on the computer that's running [!INCLUDE[webserver](../developer/includes/webserver.md)] or [!INCLUDE[server](../developer/includes/server.md)]. <br /><br /> Learn more in [Authentication and User Credential Type](../administration/users-credential-types.md).<br /><br /> Value: The subject name of the certificate.<br /><br /> Default value: none|
|GlobalEndPoints|Specifies the comma-separated list of global endpoints that are allowed to call this website. The values must include http scheme and fully qualified domain name (FQDN), such as `https://businesscentral.microsoft.com`.<br /><br />"GlobalEndPoints": "null,ms://businesscentral,ms://dynamicsnav"<br /><br /> Default value: none|
...
|LoadScriptsFromCdn|Specifies whether to load scripts from Content Distribution Networks (CDNs). This setting only applies to scripts that are available from a CDN, like jQuery.<br /><br />If set to `false`, scripts are loaded from the web server, which is useful in, for example, an intranet scenario where there's no internet access.<br />Default value: `true`<br /><br />**DISCONTINUED AFTER:** Business Central Spring 2019|
...
|OfficeSuiteShellServiceClientTimeout |Defines the time Business Central waits for the Office Suite Shell Service to respond.<br /><br />**Important:** This setting is deprecated in Business Central, and it has no effect on the Web Server instance. <br /><br /> Default value: 10|
|PersonalizationEnabled|Specifies whether personalization is enabled in the [!INCLUDE[webclient](../developer/includes/webclient.md)]. Set to `true` to enable personalization.<br /><br />Learn more in [Managing Personalization](/dynamics365/business-central/ui-personalization-manage).|
...
|RequireSsl|Specifies whether Secure Sockets Layer (SSL) (https) is required. If the value is set to `true`, all cookies are marked with a `\u0027secure\u0027` attribute. If SSL is enabled on the web server, you should set this setting to `true`.<br /><br /> Values: `true`, `false`<br /> Default value: `false`|
...
|SaveValueToDatabasePromptly|If set to `true`, changes to values in fields are saved to the database as soon as the user moves focus from the field, for example, by selecting <kbd>Tab</kbd>. `false` is the default value. If set to `false`, changes aren't saved until the user closes the page by using the back arrow or stops editing by choosing the edit (pencil) button. After a user makes changes to a field and moves focus to the next field, the status in the upper-right corner of the page changes to **Saving** and then to **Saved**. This behavior can be misleading because the changes aren't yet saved to the database at this time.|
|MaximumWebSocketsMessageSize|Specifies the maximum size, in bytes, of a single WebSocket message that the Business Central Web Server accepts from the browser client. Incoming messages that exceed this limit are rejected and the connection is closed. Use this setting to cap memory consumption per request and to mitigate denial-of-service from oversized payloads (for example, large control add-in payloads or file uploads sent inline). Default: 314572800 (300 MB)|

### `ApplicationIdSettings` element settings

|Setting/KeyName|Description|  
|-------------|-----------------|  
|BaseHelpUrl|Specifies the link to the online Help library that the deployment uses, such as *`https://mysite.com/{0}/mylibrary/`*.<br /><br /> Default value: none<br /><br />Learn more in [Configuring the Help Experience](../deployment/configure-help.md). |
