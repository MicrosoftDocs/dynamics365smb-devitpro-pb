...
> [!NOTE]
> The `getNewUsersFromOffice365Async` API isn't supported when using service-to-service (S2S) authentication. This operation requires that users are assigned the **SUPER** permission set in all companies, which can't be assigned to application users. Learn more at [Using service-to-service (S2S) authentication](/dynamics365/business-central/dev-itpro/administration/automation-apis-using-s2s-authentication).
