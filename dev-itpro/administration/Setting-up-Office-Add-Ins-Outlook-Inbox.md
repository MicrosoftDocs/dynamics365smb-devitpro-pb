...
1. Create a new app registration on your tenant with the following settings:
    ...
    |Redirect URI - URI box|Enter the base URL for your Business Central on-premises web client. For Exchange Online, the URL has the format `brk-multihub://<web server hostname>` <br /> For Exchange Server - `https://<web server hostname>`"|Exchange Online: `brk-multihub://MyBCWebServer`<br /> Exchange Server: `https://MyBCWebServer`|
