...
AL function tracing events track the execution of AL functions and statements on the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] instance.  
...
> [!IMPORTANT]  
>  If the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] instance is not configured for full AL function tracing, then only root-level AL function will be traced. Statements and AL functions that are called from functions will not be traced. By default, the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] instance is not configured for full AL functionf tracing. For information about how to specify full AL function tracing, see [Configuring Business Central Server](configure-server-instance.md).  
...
|403|ExecuteALFunction|AL statements that are executed.<br /><br /> **Important:** This trace event is only traced when the [!INCLUDE[nav_server](../developer/includes/nav_server_md.md)] is configured to full AL function tracing.|  
