...
- Another way is from Windows PowerShell. Start PowerShell 7 (or Windows PowerShell 5) as an ad ministrator. At the prompt, run the following command:
    ...
    If you're using PowerShell 7, the PowerShell 7 modules are loaded. If you're using PowerShell 5, the PowerShell 5 compatibility layer module module is loaded.
...
1. Ensure the PowerShell Remoting is enabled on the remote computer that's running [!INCLUDE[server](../developer/includes/server.md)]. From the remote computer, start PowerShell and run the following command:

   ```powershell
   Enable-PSRemoting -Force
   ```

1. On the local computer, start PowetShell 7 (or Windows PowerShell 5) as an administrator.
...
> [!IMPORTANT]
>  With 2026 release wave 1 (v28.0), remote adminstration must be done using Windows PowerShell 5.1.  When connecting via PowerShell remoting (Enter-PSSession or Invoke-Command), the default WinRM endpoint always hosts Windows PowerShell 5.1, even if PowerShell 7 is installed on the server. PowerShell 7–based remoting is not supported for the Business Central administration modules.
