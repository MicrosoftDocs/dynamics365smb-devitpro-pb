...
- Source environment settings (i.e. Update Window, Application Insights Connection String, and Security Group) set in the Administration Center at the time the restore is started will be retained on the target environment and not be restored to the values that existed on the source environment at the time that the source environment is being restored to.
