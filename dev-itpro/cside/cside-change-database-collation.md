---
title: Change Collation of an Existing Business Central
...
1. Use SQL Server Management Studio to create new databases that are configured to use the wanted collation.  
    ...
    Make sure that the service account of the [!INCLUDE[server](../developer/includes/server.md)] instance that will connect to the database has proper permission to the database. Learn more in [Provisioning the Microsoft Dynamics NAV Server Account](../deployment/provision-server-account.md).

1. Open the [!INCLUDE[adminshell](../developer/includes/adminshell.md)] as an administrator.
1. Export the application objects, application data, and tenant data from one or more original databases to a `.navdata` type file.
...
1. Import the application objects and application data from the exported file to the new application database.
...
1. Connect the new application database to the [!INCLUDE[server](../developer/includes/server.md)] instance.  

    ```powershell  
    Set-NAVServerConfiguration <server instance> -KeyName DatabaseName -KeyValue <new application database>
    ```  

    Learn more in [Connect a Server Instance to a Database](../administration/connect-server-to-database.md).
...
1. (Multitenant only) Mount the new tenant database to the server instance.
...
1. Synchronize the tenant database with the application.
...
1. Synchronize the tenant database with the published extensions.
    ...
    For [!INCLUDE[prod_short](../developer/includes/prod_short.md)] 2019 release wave 2 (version 15) and later, you might use the System Application, Business Foundation, Base Application, Application extensions. If so, synchronize these extensions first, in the listed order.
   ...
   <!--
    ...
    ```
    -->
1. Import the tenant data from the exported file to the new tenant database.
...
1. In SQL Server Management Studio, create a new database that uses the wanted collation.  
    ...
    Make sure that the service account of the [!INCLUDE[server](../developer/includes/server.md)] instance that will connect to the database has proper permission to the database. The service account must be a member of the db\_owner role of the database. Learn more in [Provisioning the Microsoft Dynamics NAV Server Account](../deployment/provision-server-account.md).  

1. Export the application objects, application data, and tenant data from the original database to a `.navdata` type file.
...
1. Import the data from the original database to the new application database by running the Import-NAVData cmdlet as shown in the following example.  
    ...
    Learn more in [Import-NAVData cmdlet](/powershell/module/microsoft.dynamics.nav.management/import-navdata).  

1. Connect the new database to the [!INCLUDE[server](../developer/includes/server.md)] instance.  

    Learn more in [Connect a Server Instance to a Database](../administration/connect-server-to-database.md). 

1. Restart the [!INCLUDE[server](../developer/includes/server.md)] instance.  
