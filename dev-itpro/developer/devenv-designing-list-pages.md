...
On mobile devices, records are only displayed as tiles. By default, the first 5 fields defined in are used in the tiles. So it is important that you configure a `fieldgroup(Brick; <Field>` control in the table code to display the desired fields. For more information, see [Field Groups](devenv-field-groups.md).
...
### Configure the actions to display for shortcut menu on rows
