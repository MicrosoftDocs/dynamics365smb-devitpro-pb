...
- The environment settings set in the Admin Center, including Application Insights connection string and update window, are copied over to the target environment. If you do not want the target environment to emit telemetry to the same Application Insights resource as the source environment you can remove or change the connection string after the copy completes or use [Data Collection Rules](/dynamics365/business-central/dev-itpro/administration/telemetry-control-cost#use-data-collection-rules-dcr) on the Application Insights resource to filter out telemetry from any environments from which you do not want to collect telemetry on that resource.
