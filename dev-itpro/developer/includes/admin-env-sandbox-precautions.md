...
- Development Extensions published from Azure DevOps and any extensions that depend on them are uninstalled when the source environment is of type Sandbox. Data created in the source environment by extensions uninstalled in the target copy will not be deleted. Any updates to per-tenant extensions slotted to be upgraded with the next environment minor or major upgrade will be retained.
