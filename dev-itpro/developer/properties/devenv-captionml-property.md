...
2. If there is no `ENU` entry, then use the value of the language entry that is first alphabetically is used. For example, if the CaptionML property is set to `DEU='Name',DAN='Navn',ESP='Nombre',FRA='Nom';`, then `DAN='Navn'` would be used, even though it is second in the list.  
