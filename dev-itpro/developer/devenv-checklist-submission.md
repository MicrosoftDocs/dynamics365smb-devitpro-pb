...
|The extension submitted should have unique `AppId`.| Every extension should have unique `AppId` and it is not allowed to submit PTEs and AppSource apps with the same `AppId`. Also see [Constraints on extension types](devenv-extension-types-and-scope.md#constraints-on-extension-types).
