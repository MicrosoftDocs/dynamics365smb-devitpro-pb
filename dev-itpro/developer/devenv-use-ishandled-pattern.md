...
Contributing code and implementing missing features eliminates the need for the events, and will improve the product.

If the code contribution is not wanted, we need to add proper extensibility, which involves adding concepts to the base application. Adding concepts lets us add more implementations. In most cases, the solution involves adding and changing multiple object and setup fields that include multiple interfaces.

A good example is payment services that are implemented in the base app. They only provide a generic concept, and then each implementation can come up with their own payment service. Another example is E-Documents.
