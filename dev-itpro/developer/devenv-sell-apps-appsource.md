---
title: Selling Business Central apps through Marketplace
description: Learn how to enable transactability in Marketplace and through CSP for Business Central apps.
...
It's also recommended to create a dummy app where you can learn and play with functionality first. Furthermore, it's recommended to use private plans only, because this hides the app in Marketplace, making it only accessible to yourself, and anyone that you have given access to, in order to the test app.

## The process of enabling transactability

The following steps outline the process of enabling transactability for your app, assuming that you already have an existing offer in Marketplace. If you don't have an existing offer in Marketplace, read more here [Create a Dynamics 365 Business Central offer](/partner-center/marketplace/dynamics-365-business-central-offer-setup). Having assured any transition period for your app by setting up side-by-side support, you can enable transactability for your app. The following outlines the steps to take:
...
![<!--alt text start -->Sample transact app with plans in Marketplace<!--alt text end -->](media/plans-small.png "Sample transact app with plans in Marketplace")
...
- Having opted into transactability, defined plans, and mapped these plans to functionality in your app by using entitlements, you can now publish your app. Your app goes through the Marketplace validation process, and once it's published, users can purchase your app directly in Marketplace.

![<!--alt text start -->Sample transact app in Marketplace<!--alt text end -->](media/converttemperatureappsource-small.png "Sample transact app in Marketplace")
...
The pricing model is a per-user model, where you can choose to define the price per user per month or per year. The plan can either be public, which means that it's shown in Marketplace, or private, which means that it's only shown to customers that the publisher has specified. Each plan can specify the supported markets, and can have market-specific pricing. The markets supported are the same as the markets supported for Business Central.
...
When the app has been updated with entitlements, it must be published to Marketplace and go through the validation process. Once it's been validated, it surfaces in Marketplace with the different plans and pricing, allowing customers to purchase directly in the Marketplace. Once licenses have been purchased, they appear in the customers Microsoft 365 admin center, and can be assigned to individual users. For more information, see [Use the Licenses page to assign licenses to users](/microsoft-365/admin/manage/assign-licenses-to-users?view=o365-worldwide&preserve-view=true#use-the-licenses-page-to-assign-licenses-to-users).
...
The side-by-side support is the ability to fall back to custom licensing for existing customers in a transition period. It's crucial that you implement side-by-side support for your app before opting into transactability on Marketplace. To enable side-by-side support, you must create an entitlement, which maps the user to the required permission sets to run their custom licensing. On the entitlement object, you must set the `Type` property to `Unlicensed`. For more information, see [Entitlement object](devenv-entitlement-object.md). 
...
[Sell Business Central apps through Marketplace - release plan](/dynamics365/release-plan/2023wave2/smb/dynamics365-business-central/sell-business-central-apps-through-appsource)  
