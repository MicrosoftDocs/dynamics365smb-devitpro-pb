...
|schemaUpdateMode|No|Specifies the data synchronization mode when you publish an extension to the development server, for example: <br>`"schemaUpdateMode": "Recreate"`</br> The default value is Synchronize. Learn more in [Retaining table data after publishing](devenv-retaining-data-after-publishing.md)  <br>[!INCLUDE[nav_not_supported](includes/nav_not_supported.md)]  |
...
|snapshotFileName|No|Specifies the snapshot file name used when snapshot debugging files are saved. Learn more in [Snapshot Debugging](devenv-snapshot-debugging.md).|
|primaryTenantDomain|No|Specifies the URL of the Microsoft Entra organization or company associated with the Microsoft Entra tenant. This setting enables Microsoft Entra ID scenarios for on-premises installations. Learn more in [Microsoft Entra authentication for Business Central on-premises
...
The attach configuration is used for specific debugging scenarios where you don't want to publish and invoke functionality to debug it. Learn more in [Attach and debug next](devenv-attach-debug-next.md).
...
Snapshot debugging allows you to record AL code that runs on the server, and when it has completed, you can debug the recorded *snapshot* in Visual Studio Code. Learn more in [Snapshot debugging](devenv-snapshot-debugging.md).
...
|userId|No| The GUID of the user who initiated the process to start snapshot debugging. For on-premises, this can also be the user name in user password authentication scenarios. The user must be able to start, or have a session type opened that is specified in the `breakOnNext` parameter. <br>**Note:** Specifying `userId` doesn't work with Windows authentication: `"authentication" : "Windows"`, in which case you can only choose `sessionId` or attach to the next session. Learn more in [Launch JSON file](devenv-json-launch-file.md).|
