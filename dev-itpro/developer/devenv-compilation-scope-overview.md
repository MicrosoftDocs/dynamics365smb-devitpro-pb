...
In the manifest of an extension (the app.json file), you can set the **target** property to specify the compilation target of an extension. The available values are `OnPrem` and `Cloud`.

The `target` property informs the compiler, which APIs can be used within the current project. The following rules apply:
