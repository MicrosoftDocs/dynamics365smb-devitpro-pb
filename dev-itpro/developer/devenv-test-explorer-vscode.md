...
You can use the built-in *Test Explorer* in Visual Studio Code to discover and run AL tests in your workspace. This feature lets you run tests directly from the IDE instead of using an external test runner or the Business Central web client.
...
- Business Central version 28.0 or newer (2026 release wave 1).
- The AL Language extension for Visual Studio Code is installed.
- Your tests are published to a Business Central server (sandbox or local).

## Discover and run tests

The Test Explorer automatically detects test codeunits and test methods in the active workspace.

1. Open the **Test Explorer** view in Visual Studio Code. Learn more about the Test Explorer at [Testing in Visual Studio Code](https://code.visualstudio.com/docs/debugtest/testing).
...
> [!IMPORTANT]
> You must publish your tests to a Business Central server before you can run them. The Test Explorer doesn't compile or publish tests for you.
...
Running tests from Visual Studio Code is supported on Business Central version 28.0 and newer on the following environments:

- Online sandbox environments.
- Local (on-premises) server installations.

> [!NOTE]
> Running tests on production environments isn't supported because it might affect business operations. Learn more at [Testing the application overview](devenv-testing-application.md).
