...
While you could change the existing NAV source code using C/SIDE, the best approach to developing with the AL Language Development Environment is to work with [Extensions](devenv-dev-overview) only. That means that you can extend the existing code by adding new functionality or integrating your code using [Events](devenv-events-in-al), but it is not possible to change existing code, for example in codeunits. 
