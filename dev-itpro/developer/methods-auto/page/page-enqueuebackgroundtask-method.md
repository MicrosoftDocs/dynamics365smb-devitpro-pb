...
​The number of page background tasks that can be run simultaneously for a parent session is controlled by the Child Session Max Concurrency server configuration setting (which has a default value of 5). If this number is exceeded, then they will be queued and run when a slot becomes available as other tasks are finished. Enqueuing the task will fail if the total number of enqueued tasks exceed the Child Sessions Max Queue Length server configuration setting.​
...
It is possible to enqueue the task again in the completion trigger, but this is in general not recommended as it can lead to an endless loop. 
