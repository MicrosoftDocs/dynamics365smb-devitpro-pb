...
It's, however, not possible to allow both, debugging and **Go to Definition**, and still protect source from being extracted through the debug experience, for example, by using third party Visual Studio Code tools. For AppSource apps, if you want to protect your IP, it's recommended to limit access to the source by setting the `resourceExposurePolicy` flags to `false`. Then rely on the ability to grant yourself and optionally trusted reseller partners time-limited individual access through the dynamic override of the resource policy. For more information, see [Overriding the resource policy](devenv-security-settings-and-ip-protection.md#overriding-the-resource-policy) in this article.
