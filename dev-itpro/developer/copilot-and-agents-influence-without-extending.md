...
Some AI-powered features are directly extensible using AL and offer documented, prescribed mechanisms to modify or enhance existing features. This article covers a different class of Copilot and agent capabilities that, while not directly extensible, are still able to work with the custom code and content originating from your extensions. All extensions installed to an environment have the opportunity to influence the output of Copilot and agents.
...
|-|-|-|-|
|Analyze list|No |Yes |Copilot automatically works with fields you add to existing pages, and new list or worksheet page objects from your extension. No action is required to enable this.|
|Chat|No |Yes|Copilot can query new pages and tables from your extension, and can include, display or filter on fields you add to existing pages. No action is required to enable this.<br><br>The following types of fields are not supported by Copilot: any field without a tooltip, fields with AL property AllowInCustomizations set to false, and fields of type Blob, RecordId, InStream, OutStream, Media, MediaSet, Notification, JsonArray, JsonObject, JsonToken, JsonValue, Variant, and SecretText. <br><br>While in preview, Copilot is not able to provide guidance or answer questions based on documentation for your extensions. This functionality is coming soon <link to RP>.|
|Autofill|No|Yes |Copilot offers to autofill fields you add to existing pages, and fields from new card or document page objects from your extension. No action is required to enable this.<br><br>See <availability> for other limitations.  |
|Summarize|No |Yes |CCopilot generates summaries that may include any fields, FactBoxes, actions that use RunObject to lead to other pages, that you add to existing pages, or new card and document page objects from your extension. No action is required to enable this.| 
