...
|Summarize|No |Yes |Copilot generates record summaries that include fields, FactBoxes, and actions from your extension. The fields can be added by the extension to existing pages or new card and document page objects. Actions must use the [RunObject property](properties/devenv-runobject-property.md) to open other pages to be included. No action is required for Copilot to work with your extensions. Copilot omits from summaries any field or FactBox that you hide by setting the [Visible property](properties/devenv-visible-property.md) to `false`.|
