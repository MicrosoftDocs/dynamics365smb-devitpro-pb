...
|---------------|---------------------------------------|  
|`http&#124;https`|Specifies the Internet protocol to use. Valid options are `http` and `https`.<br /><br /> The `https` protocol helps secure the Microsoft Dynamics NAV data that is transmitted over the Internet. To use `https`, Secure Sockets Layer \(SSL\) must be enabled on the connection to the [!INCLUDE[webclient](includes/webclient.md)]. For more information, see [Configuring SSL to Secure the [!INCLUDE[prodshort](../developer/includes/prodshort.md)] Web Client Connection](../deployment/configure-ssl-web-client-connection.md).|  
