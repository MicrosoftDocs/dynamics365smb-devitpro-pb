...
> [!NOTE]  
> The event recorder captures all events that are raised in the same session. If the actions performed by the user are in another session, then the event recorder doesn't capture them.
...
> [!NOTE]  
> The recorded events aren't saved. When you refresh the page, the recorded events disappear.

Learn more about how to subscribe to events in [Subscribing to events](devenv-subscribing-to-events.md).
