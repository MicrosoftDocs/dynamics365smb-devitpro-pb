...
|aadtenantid|The unique identifier for an Microsoft Entra tenant. The value can be formatted as a GUID or domain name. This is useful to those who work across multiple Microsoft Entra ID organizations, such as delegated administrators, support personnel or external accountants, because it allows explicitly targeting an Microsoft Entra ID tenant. If this is omitted, you will be directed to the primary Microsoft Entra ID tenant or the same Microsoft Entra ID tenant that you are currently signed in to.|`ms-businesscentral://businesscentral.mysolution.com/mysolutionaadtenant.onmicrosoft.com`|
