...
Modification requests (like `POST`, `PATCH`, `PUT`, or `DELETE`) only support `ReadWrite` as a value for data access intent. If you try to specify `Data-Access-Intent: ReadOnly` for such requests, the API returns an error.
