...
The AL Language extension has many settings that can be defined for a specific user or for a workspace. To activate the settings, press <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd>, and then choose **Preferences: Open Settings (UI)** for workspace settings, or choose **Preferences: Open User Settings** for user settings. Under **Extensions**, and **AL Language extension configuration**, you find the settings that are available for the AL Language extension. If a setting isn't available through the UI, you can edit it directly in the `settings.json` file. For tips on how to optimize Visual Studio Code, see [Optimizing Visual Studio Code for AL Development](devenv-optimize-visual-studio-code.md).
...
|Incognito|Specifies whether to open the browser in Incognito/InPrivate mode when launching the application from Visual Studio Code. This option takes effect only if the **Browser** option is set to a nondefault value.|
...
|NugetFeeds|Array of custom NuGet feed URLs to search for symbol packages. These feeds are checked after the built-in Microsoft feeds. Each URL should point to a public NuGet v3 feed that doesn't require authentication. Syntax is `"al.nugetFeeds": ["https://pkgs.dev.azure.com/myorg/myproject/_packaging/myfeed/nuget/v3/index.json"]`.<br>Used with the `al.useOnlyCustomFeeds` and `al.symbolsCountryRegion` settings.|
|Output Analyzer Statistics|Specifies whether code analyzer performance statistics are output when running a build.|
...
|symbolsCountryRegion|Country/region code for localized symbol packages. Use ISO 3166-1 alpha-2 codes (for example, `us`, `de`, `dk`) or `w1` for worldwide. If not specified, you'll be prompted during download. Syntax is `"al.symbolsCountryRegion": "us"`.<br>Used with the `al.nugetFeeds` and `al.useOnlyCustomFeeds` settings.|
|useOnlyCustomFeeds|Boolean. When set to `true`, only the feeds specified in `al.nugetFeeds` are used. Built-in Microsoft feeds are ignored. Syntax is `"al.useOnlyCustomFeeds": true`. Used with the `al.nugetFeeds` and `al.symbolsCountryRegion` settings.|
