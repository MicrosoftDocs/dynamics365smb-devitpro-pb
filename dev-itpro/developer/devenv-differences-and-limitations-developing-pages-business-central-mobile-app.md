...
|Search across list columns|Partly supported. Search won't include FlowFields.|Partly supported. Search won't include FlowFields.|On the `Customer` list page.||  
...
|File download|Available. Can't download multiple files at the same time.|Available. Can't download multiple files at the same time.|`Trial Balance` report in the **Print to Excel** check box.||  
...
|Lists|Available.|Available, with the difference that these are displayed in a brick layout with many differences and limitations. For an overview, see [Displaying Data as Tiles](devenv-lists-as-tiles.md).|Customers or Sales Orders pages.||  
