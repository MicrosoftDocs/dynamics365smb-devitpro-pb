...
|N/A|Set the property according to the .NET numeric format syntax, for example: `'<FormatString>#,##0.00;(#,##0.00);Zero'`.|Use this option to use the standard .NET numeric formatting system. Learn more in [Custom numeric format strings](/dotnet/standard/base-types/custom-numeric-format-strings).<br/><br/>**Note:** `FormatString>`is supported from version 26.|
