...
|N/A|Set the property according to the following syntax: <br/><br/>`'<FormatString>#,##0.00;(#,##0.00);Zero'`<br/><br/>|Use this option to write the number with two decimals and enclose the number in parenthesis if negative and 'Zero' if the value is `0`. This uses the standard .NET numeric formatting system. Learn more in [Custom numeric format strings](/dotnet/standard/base-types/custom-numeric-format-strings).<br/><br/>**Note:** `FormatString>`is supported from version 26.|
