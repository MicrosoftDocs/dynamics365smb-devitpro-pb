---
title: Scenarios for moving tables or fields
...
2. The move is performed on an OnPrem instance or a sandbox by a series of lifecycle operations
...
Depending on the relationship between the source and destination, three scenarios can be defined:
...
- Lateral Move - The destination extension is neither a dependent nor a dependency of the source

Each scenario requires slightly different steps in the each of the steps of the development process.

## Move Down

### Extension Structure

It is possible to move a table or a field to an existing or new dependency.
...
This can be skipped if `Dependency` is a direct dependency and the 
...
4. Publish the extension to the environment via VSCode.
...
1. Create `MyTable` in Dependency with the same ID and structure as it had in Extension Dependent. Non-breaking changes can be added between moves (eg. adding a field).
...
It is possible to move a table or a field to an existing or new dependent extension. We assume an extension `Dependency` with the table `MyTable` and an extension which is a dependent named `Dependent`. We also assume both start with a version of '1.0.0.0'.
...
1. Create `MyTable` in `Dependent` with the same ID and structure as it had in `Dependency`. Non-breaking changes can be added between moves (eg. adding a field).
...
A lateral move requires the same process as [Move Up](#move-up). However, further changes may be required to deal with objects that the table or field that is being moved depends on, eg. enums used as types of fields.
This is because after a lateral move, it must be possible to resolve the moved table or field in the both the source and destination extensions. See [Handling dependencies of moved tables or fields](#handling-dependencies-of-moved-tables-or-fields) for more details.
...
The rule of thumb when publishing extensions that perform a move to AppSource is to first submit the source extension and then the destination extension.  The publish orders from previous sections can also be used as the submission order for submitting to AppSource. Moves are allowed within a single submission or across submissions. For example, in a move down scenario it is possible to submit the source extension which moves a table and the destination dependency as a library app. It is also possible to submit them in two separate submissions.

### Validation across target releases

When submitting an extension with any move, its validity will be checked across all the possible releases of Business Central where this extension could be installed. For example, if the `application` property in the
extension is '24.0.0.0' and the latest release of Business Central is '26.0.0.0', then the move will need to be valid across all of these releases.

## Handling dependencies of moved tables or fields

### Problem Overview

When moving a table or a field, they may depend on other objects which will need to be moved in tandem. In the example below, it is not possible to move the table using the lateral scenario without also moving
the enum as then it would be impossible to resolve the type of its field.
...
In order for the move to be successful, the enum needs to be resolvable in both the source and destination extensions. This can be achieved by creating a common dependency extension for both the source
and destination and then moving the dependent objects to that extension.
...
When tables are moved out, their files can be moved to a directory of moved objects and keeping them there. It is not yet possible to remove these definitions, but grouping them together will allow for this in the future.

### Moving a field

When fields are moved out, a new table extension can be created in the source extension to contain only the moved fields. This ensures that they can be removed more easily in the future and that they don't clutter the code of fields which are not moved.
