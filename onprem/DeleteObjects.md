 ...
 *servername*  
 ...
 The name of the database server that hosts the [!INCLUDE[navnow](includes/navnow_md.md)] database that contains the objects that you want to delete. The format of this parameter is \<*computername*>\\\<*database server instance*>.  
 ...
 *filter*  
 ...
 A filter on the **Object** table. Use the *filter* parameter to specify which objects to delete. Use a semicolon to separate multiple filters. The only operator that you can use in the filter is '='. The following table shows the fields in the **Object** table on which you can filter.  
...
|Locked By|Domain\\username|"Locked By=CRONUS\\viktor"|  
 ...
 If you do not specify a filter, then all objects in the **Object** table are deleted.  
 ...
 *navservername*  
 ...
 Use this parameter only when you are deleting table objects and the *validatetablechanges* is set to **no**.  
 ...
 *tenant*  
 ...
 Specifies the ID of the tenant that is accessed when you run objects from the development environment. If your solution is not set up to deploy in a multitenant deployment architecture, then leave the parameter empty.  
